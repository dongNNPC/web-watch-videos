 ...
 * 
 */
@NamedQueries({ @NamedQuery(name = "Video.ASC", query = "SELECT v FROM Video v  ORDER BY v.views  ASC"),
...
@NamedNativeQueries({
		@NamedNativeQuery(name = "Report.random10", query = "SELECT TOP (:id) * FROM Videos ORDER BY newid()", resultClass = Video.class) })
@Entity
@Table(name = "Videos")
